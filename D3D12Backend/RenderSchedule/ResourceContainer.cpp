...
namespace Boolka
{
    ...
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::Tex);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::Buf);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::SRV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::RTV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::DSV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::VBV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::IBV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::RootSig);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::DescHeap);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::FlipBuf);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::FlipCBV);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::FlipUploadBuf);
    BLK_DEFINE_ENUM_OPERATOR_PLUS(ResourceContainer::DefaultRootSigBindPoints);
    ...
    bool ResourceContainer::Initialize(Device& device, RenderEngineContext& engineContext, DisplayController& displayController, ResourceTracker& resourceTracker)
    {
        ...
        for (UINT i = 0; i < BLK_IN_FLIGHT_FRAMES; ++i) GetFlippableBuffer(i, FlipBuf::ShadowMap).Initialize(device, shadowMapCbSize, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
        ...
        for (UINT i = 0; i < BLK_IN_FLIGHT_FRAMES; ++i) GetFlippableUploadBuffer(i, FlipUploadBuf::ShadowMap).Initialize(device, shadowMapCbSize);
        ...
        for (UINT i = 0; i < BLK_IN_FLIGHT_FRAMES; ++i) GetFlippableBuffer(i, FlipBuf::ShadowMap).SetDebugName(L"ShadowMapConstantBuffer%d", i);
        ...
        for (UINT i = 0; i < BLK_IN_FLIGHT_FRAMES; ++i) GetFlippableUploadBuffer(i, FlipUploadBuf::ShadowMap).SetDebugName(L"ShadowMapConstantUploadBuffer%d", i);
#endif
