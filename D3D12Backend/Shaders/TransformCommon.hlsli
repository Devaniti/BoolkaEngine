...
float3 CalculateViewPos(float2 UV, float depthVal)
{
    ...
    float4 viewPos = mul(viewProjPos, Frame.invProjMatrix);
    return viewPos.xyz / viewPos.w;
}

float3 CalculateWorldPos(float3 viewPos)
{
    return mul(float4(viewPos, 1.0f), Frame.invViewMatrix).xyz;
}

float3 CalculateWorldSpaceNormal(float3 vsNormal)
{
    return normalize(mul(float4(vsNormal, 0.0f), Frame.invViewMatrix).xyz);
}
