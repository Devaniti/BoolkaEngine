...
namespace Boolka
{
    ...
    bool Texture1D::Initialize(ID3D12Resource* resource)
    {
        BLK_ASSERT(m_Resource == nullptr);
#ifdef BLK_DEBUG
        D3D12_RESOURCE_DESC resourceDesc = resource->GetDesc();
        BLK_ASSERT(resourceDesc.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE1D);
#endif
    ...
    bool Texture1D::Initialize(Device& device, D3D12_HEAP_TYPE heapType, UINT64 width,
                               ...
                               D3D12_RESOURCE_STATES initialState, UINT16 arraySize /*= 1*/)
    {
        ...
        D3D12_RESOURCE_DESC resourceDesc =
            FillDesc(width, mipCount, format, resourceFlags, arraySize);
    ...
    bool Texture1D::Initialize(Device& device, ResourceHeap& resourceHeap, size_t heapOffset,
                               UINT64 width, UINT16 mipCount, DXGI_FORMAT format,
                               D3D12_RESOURCE_FLAGS resourceFlags, D3D12_CLEAR_VALUE* clearValue,
                               D3D12_RESOURCE_STATES initialState, UINT16 arraySize /*= 1*/)
    {
        D3D12_RESOURCE_DESC resourceDesc =
    ...
    void Texture1D::GetRequiredSize(size_t& outAlignment, size_t& outSize, Device& device,
                                    UINT64 width, UINT16 mipCount, DXGI_FORMAT format,
    ...
    size_t Texture1D::GetUploadSize(UINT width, UINT16 mipCount, DXGI_FORMAT format,
                                    D3D12_RESOURCE_FLAGS resourceFlags, UINT16 arraySize /*= 1*/)
    {
        size_t result = 0;
        ...
        for (UINT16 currentMip = 0; currentMip < mipCount; ++currentMip)
        {
            ...
            size_t textureSize = BLK_CEIL_TO_POWER_OF_TWO(rowPitch,
    ...
    UINT Texture1D::GetBPP(DXGI_FORMAT format)
    {
        switch (format)
        {
    ...
    D3D12_RESOURCE_DESC Texture1D::FillDesc(UINT64 width, UINT16 mipCount, DXGI_FORMAT format,
                                            D3D12_RESOURCE_FLAGS resourceFlags, UINT16 arraySize)
    {
        D3D12_RESOURCE_DESC resourceDesc = {};
