...
namespace Boolka
{
    ...
    bool ComputeCommandAllocator::InitializeCommandList(ComputeCommandListImpl& commandList, Device& device, ID3D12PipelineState* PSO)
    {
        return commandList.Initialize(device, m_CommandAllocator, PSO);
    }

    bool ComputeCommandAllocator::ResetCommandList(ComputeCommandListImpl& commandList, ID3D12PipelineState* PSO)
    {
