...
namespace Boolka
{
    ...
    bool ShadowMapRenderPass::Render(RenderContext& renderContext, ResourceTracker& resourceTracker)
    {
        ...
        Matrix4x4 uploadData[BLK_MAX_LIGHT_COUNT * BLK_TEXCUBE_FACE_COUNT];
        ...
        UINT height = BLK_SHADOWMAP_SIZE;
        UINT width = BLK_SHADOWMAP_SIZE;
        ...
        for (size_t lightIndex = 0; lightIndex < lights.size(); ++lightIndex)
        {
            ...
            for (size_t faceIndex = 0; faceIndex < BLK_TEXCUBE_FACE_COUNT; ++faceIndex)
            {
                ...
                auto& shadowMapDSV = resourceContainer.GetDSV(ResourceContainer::DSV::ShadowMap0 + lightIndex * BLK_TEXCUBE_FACE_COUNT + faceIndex);
                ...
                engineContext.GetScene().GetBatchManager().Render(commandList, BatchManager::BatchType::ShadowMap0 + resourceIndex);
            }
        }
