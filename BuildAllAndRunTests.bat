...
(%InstallDir%\Common7\IDE\devenv.exe BoolkaEngine.sln /Build "Development" && echo Successfully built Development configuration) || echo Building Development configuration failed && exit /b 1
...
(%InstallDir%\Common7\IDE\CommonExtensions\Microsoft\TestWindow\vstest.console.exe x64\Development\BoolkaCommonUnitTests.dll && echo Tests passed on Development configuration) || echo Tests failed on Development configuration && exit /b 1
